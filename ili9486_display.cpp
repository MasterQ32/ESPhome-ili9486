...
void ILI9486Display::setup_memory_() {
  size_t len = this->get_buffer_length_();
  ESP_LOGI(TAG, "Initialize memory to %llu bytes", len);
  this->init_internal_(len);
}
...
void ILI9486Display::display_() {
  ...
  for (uint16_t row = 0; row < h; row++) {
    for (uint16_t col = 0; col < w; col++) {
      ...
      // HACK: 
      uint16_t color = color332_from_color565( this->buffer8()[pos] );
...
uint16_t ILI9486Display::color565_from_color332(uint8_t color_8bit) {
  int r = color_8bit >> 5;
...
uint8_t ILI9486Display::color332_from_color565(uint16_t color_16bit) {
...
uint8_t ILI9486Display::color332_from_color(Color color) {
...
uint16_t ILI9486Display::color565_from_color(Color color) {
  // convert 32bit color to 8 bit buffer
  uint8_t r = color.r >> 3;  // 5
  uint8_t g = color.g >> 2;  // 6
  uint8_t b = color.b >> 3;  // 5
...
void ILI9486Display::fill_internal_(Color color) {
  ...
  uint16_t color565 = color565_from_color(color);
  for (uint32_t i = 0; i < (this->get_width_internal()) * (this->get_height_internal()); i++) {
...
void HOT ILI9486Display::draw_absolute_pixel_internal(int x, int y, Color color) {
  ...
  // HACK:
  this->buffer8()[pos] = color332_from_color(color);
}

// should return the total size: return this->get_width_internal() * this->get_height_internal() * 2 // 16bit color
// values per bit is huge
uint32_t ILI9486Display::get_buffer_length_() {
  ESP_LOGI(TAG, "internal width: %lu", this->get_width_internal());
  ESP_LOGI(TAG, "internal height: %lu", this->get_height_internal());
